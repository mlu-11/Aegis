...
import { Add, PlayArrow, Stop, CheckCircle, Visibility } from '@mui/icons-material';
...
const SprintList: React.FC = () => {
  ...
  const { getSprintsByProject, addSprint, updateSprint, deleteSprint, canStartSprint } = useSprintStore();
  ...
  const handleSprintFormSubmit = (sprintData: Omit<Sprint, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      if (editingSprint) {
        ...
        if (sprintData.issueIds && sprintData.issueIds.length > 0) {
          ...
          const newSprint = allSprints.find(s => s.name === sprintData.name && s.projectId === sprintData.projectId);
          
          if (newSprint) {
            sprintData.issueIds.forEach(issueId => {
              assignIssueToSprint(issueId, newSprint.id);
            });
          }
        }
        
        showSnackbar(`Sprint created successfully with ${sprintData.issueIds?.length || 0} issues!`, 'success');
      }
    } catch (error) {
    ...
    if (deletingSprint) {
      try {
        ...
        sprintIssues.forEach(issue => {
          const { assignIssueToSprint } = useIssueStore.getState();
          assignIssueToSprint(issue.id, undefined);
        });
        
        deleteSprint(deletingSprint.id);
        showSnackbar('Sprint deleted successfully! All issues moved back to backlog.', 'success');
      } catch (error) {
  ...
  const handleSprintStatusChange = (sprintId: string, newStatus: string) => {
    if (newStatus === 'ACTIVE' && projectId && !canStartSprint(projectId)) {
      showSnackbar('Cannot start sprint: Another sprint is already active', 'error');
      return;
    }

    updateSprint(sprintId, { status: newStatus as any });
  ...
  const ganttTasks: Task[] = useMemo(() => {
    return projectSprints.map((sprint) => {
      ...
      return {
        ...
        isDisabled: false,
        styles: {
          progressColor: sprint.status === 'COMPLETED' ? '#4caf50' : 
                        sprint.status === 'ACTIVE' ? '#2196f3' : '#ff9800',
          progressSelectedColor: sprint.status === 'COMPLETED' ? '#388e3c' : 
                                sprint.status === 'ACTIVE' ? '#1976d2' : '#f57c00',
        },
      };
    });
          ...
          <CardContent className="p-0">
            {ganttTasks.length > 0 ? (
              <Gantt
                ...
                columnWidth={viewMode === ViewMode.Month ? 300 : viewMode === ViewMode.Week ? 100 : 65}
                rowHeight={50}
                listCellWidth="250px"
              />
            ) : (
                ...
                <Paper className="p-4 hover:shadow-md transition-shadow">
                  ...
                  <Box className="mb-3">
                    <Box className="flex items-center justify-between mb-1">
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {sprintIssues.filter(issue => issue.status === 'DONE').length} / {sprintIssues.length} issues completed
                      </Typography>
                    </Box>
                    <Box className="w-full bg-gray-200 rounded-full h-2">
                      <Box 
                        ...
                        style={{ 
                          width: `${progress}%`,
                          backgroundColor: sprint.status === 'COMPLETED' ? '#4caf50' : 
                                          sprint.status === 'ACTIVE' ? '#2196f3' : '#ff9800'
                        }}
                      />
                    </Box>
                  ...
                  <Box className="flex items-center justify-between">
                    <Typography variant="body2" color="text.secondary">
                      {sprintIssues.length} issues • Duration: {Math.ceil((sprint.endDate.getTime() - sprint.startDate.getTime()) / (1000 * 60 * 60 * 24))} days
                    </Typography>
                    ...
                    <Box className="flex gap-2">
                      ...
                      {sprint.status === 'PLANNING' && (
                        <Button
                          variant="contained"
                          size="small"
                          disabled={projectId ? !canStartSprint(projectId) : false}
                          onClick={() => handleSprintStatusChange(sprint.id, 'ACTIVE')}
                          title={projectId && !canStartSprint(projectId) ? 'Another sprint is already active' : ''}
                        >
                          Start Sprint
                        </Button>
                      )}
