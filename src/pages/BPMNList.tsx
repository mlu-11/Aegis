...
const BPMNList: React.FC = () => {
  ...
  const { getDiagramsByProject, addDiagram, deleteDiagram } = useBPMNStore();
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLElement>, diagramId: string) => {
      ...
      {diagrams.length === 0 ? (
        <Box className="text-center py-12">
          ...
          <Typography variant="body2" color="textSecondary" className="mb-6">
            Create your first BPMN diagram to start modeling your business processes
          </Typography>
          <Button
            variant="contained"
            startIcon={<Add />}
              ...
              <Card className="h-full">
                ...
                <CardActions>
                  <Button
                    ...
                    onClick={() => navigate(`/project/${projectId}/bpmn/${diagram.id}/view`)}
                  >
                    View
                  </Button>
                  <Button
                    size="small"
                    startIcon={<Edit />}
                    onClick={() => navigate(`/project/${projectId}/bpmn/${diagram.id}`)}
                  >
                    Edit
                  </Button>
                </CardActions>
